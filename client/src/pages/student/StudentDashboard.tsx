...
export const StudentDashboard: React.FC = () => {
  ...
  const fetchDashboard = () => {
    ...
    Promise.all([
      ...
      api.get('/student/granular-skills').catch(() => ({ data: null })),
      api.get('/student/roadmap').catch(() => ({ data: null })),
    ])
      .then(([dashRes, granRes, roadRes]) => {
        setData(dashRes.data);
        setGranularProfile(granRes.data);
        setRoadmapData(roadRes.data);
        const careerInts = dashRes.data?.careerInterests || [];
        if (careerInts.length > 0) {
          setPrimaryInterest(dashRes.data?.primaryCareerInterest || careerInts[0]);
          setSecondaryInterests(
            careerInts.filter((i: string) => i !== (dashRes.data?.primaryCareerInterest || careerInts[0]))
          );
        } else {
          setPrimaryInterest('');
          setSecondaryInterests([]);
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      {/* ========================================================================= */}
      {/* 0. GRANULAR SKILL INTELLIGENCE & AI ROADMAP SUMMARY CARD                  */}
      {/* ========================================================================= */}
      {granularProfile && (
        <div className="bg-white dark:bg-[#121824] rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-brand-50 dark:bg-brand-900/40 border border-brand-200 dark:border-brand-700/50 flex items-center justify-center text-brand-600 dark:text-brand-400">
                <Sparkles className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-base font-bold text-slate-900 dark:text-white">Granular Skill Intelligence & AI Learning Roadmap</h2>
                <p className="text-xs text-slate-600 dark:text-slate-400">Target Career: <strong className="text-brand-600 dark:text-brand-300">{granularProfile.targetCareer || 'Panchakarma Clinical Specialist'}</strong></p>
              </div>
            </div>

            <Link
              to="/student/skill-mapping"
              className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-brand-600 hover:bg-brand-500 rounded-xl transition-all shadow-md shadow-brand-900/30 shrink-0"
            >
              <span>Explore AI Roadmap & Tree</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            <div className="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase block">Overall Score</span>
              <span className="text-xl font-extrabold text-slate-900 dark:text-white">{granularProfile.overallSkillScore}%</span>
              <span className="text-[10px] text-brand-600 dark:text-brand-400 block font-semibold">{granularProfile.overallProficiency}</span>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase block">Strong Skills</span>
              <span className="text-xl font-extrabold text-emerald-600 dark:text-emerald-400">{granularProfile.strongSkills?.length || 0}</span>
              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 block font-semibold">≥ 75% Score</span>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase block">Improvement Areas</span>
              <span className="text-xl font-extrabold text-amber-600 dark:text-amber-400">{granularProfile.improvementSkills?.length || 0}</span>
              <span className="text-[10px] text-amber-600 dark:text-amber-400 block font-semibold">60% – 74%</span>
            </div>

            <div className="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 text-center">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase block">Critical Gaps</span>
              <span className="text-xl font-extrabold text-rose-600 dark:text-rose-400">{granularProfile.criticalGaps?.length || 0}</span>
              <span className="text-[10px] text-rose-600 dark:text-rose-400 block font-semibold">&lt; 60% (Roadmap)</span>
            </div>
          </div>

          {roadmapData?.nextActionableSkill && (
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-brand-50 dark:bg-brand-900/60 text-brand-700 dark:text-brand-300 border border-brand-200 dark:border-brand-700/50">
                  NEXT MILESTONE
                </span>
                <span className="text-slate-700 dark:text-slate-200">
                  Phase {roadmapData.nextActionableSkill.phaseNumber}: <strong>{roadmapData.nextActionableSkill.subSkillName}</strong> ({roadmapData.nextActionableSkill.currentScore}% &rarr; {roadmapData.nextActionableSkill.targetScore}%)
                </span>
              </div>
              <Link to="/student/skill-mapping" className="text-brand-600 dark:text-brand-400 font-bold hover:underline shrink-0">
                Action Plan &rarr;
              </Link>
            </div>
          )}
