...
export const StudentInternships: React.FC = () => {
  ...
  const [recommendedInternships, setRecommendedJobs] = useState<any[]>([]);
  const [allInternships, setAllJobs] = useState<Opportunity[]>([]);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [dashRes, recJobsRes, allInternshipsRes] = await Promise.all([
        api.get('/student/dashboard'),
        api.get('/student/recommended-internships'),
        api.get('/opportunities?type=INTERNSHIP'),
      ]);
      ...
      setAllJobs(allInternshipsRes.data || []);
    } catch (err) {
  ...
  const displayedRecommended = filterList(recommendedInternships, true);
  const displayedAll = filterList(allInternships, false);
    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900">Industry Internships & Practical Training</h1>
          <p className="text-xs text-slate-500 mt-1">
            Discover experiential learning internships strictly personalized to your career pathway, degree, and skills.
          </p>
        </div>
      ...
      <div className="flex items-center justify-between border-b border-slate-200">
        <div className="flex gap-2">
          ...
          >
            ...
            Explore All Platform Internships ({allInternships.length})
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        displayedRecommended.length > 0 ? (
          ...
          <EmptyState
            icon={Briefcase}
            title="No internship postings matching your current career pathway"
