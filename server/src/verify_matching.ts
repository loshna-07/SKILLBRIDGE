...
async function verifyMatchingEngine() {
  ...
  if (!s1Reg.ok) throw new Error('Student 1 registration failed: ' + JSON.stringify(s1Reg.data));
  const s1Token = s1Reg.data.token;
  const s1Profile = await prisma.studentProfile.findUnique({ where: { userId: s1Reg.data.user.id } });
  ...
  if (!s2Reg.ok) throw new Error('Student 2 registration failed: ' + JSON.stringify(s2Reg.data));
  const s2Token = s2Reg.data.token;
  const s2Profile = await prisma.studentProfile.findUnique({ where: { userId: s2Reg.data.user.id } });
  ...
  if (!internRes.ok) throw new Error('Internship creation failed: ' + JSON.stringify(internRes.data));
  ...
  if (!jobRes.ok) throw new Error('Job creation failed: ' + JSON.stringify(jobRes.data));
  ...
  if (!progRes.ok) throw new Error('Learning program creation failed: ' + JSON.stringify(progRes.data));
  ...
  if (!s1InternMatch.ok) throw new Error('S1 Internship matching failed: ' + JSON.stringify(s1InternMatch.data));
  const s1M = s1InternMatch.data;
  console.log('  -> Student 1 (Arjun) Internship Match:');
  console.log('     * Match Percentage:', s1M.matchPercentage + '%');
  console.log('     * Matched Skills:', s1M.matchedSkills.map((s: any) => `${s.name} (${s.proficiency})`));
  ...
  if (!s2InternMatch.ok) throw new Error('S2 Internship matching failed: ' + JSON.stringify(s2InternMatch.data));
  ...
  console.log('     * Skill Gaps:', s2M.skillGaps.map((g: any) => `${g.name}: ${g.gapType} (${g.severity})`));
  ...
  if (!s1JobMatch.ok) throw new Error('S1 Job matching failed: ' + JSON.stringify(s1JobMatch.data));
  const s1Job = s1JobMatch.data;
  console.log('  -> Student 1 Job Match:');
  console.log('     * Match Percentage:', s1Job.matchPercentage + '%');
  console.log('     * Matched Skills:', s1Job.matchedSkills.map((s: any) => s.name));
  console.log('     * Missing Skills:', s1Job.missingSkills.map((s: any) => s.name));
  console.log('     * Eligibility isEligible:', s1Job.eligibility.isEligible);
  console.log('     * Skill Gaps:', s1Job.skillGaps.map((g: any) => `${g.name}: ${g.gapType} (${g.severity})`));
  ...
  const toxiGap = s1Job.skillGaps.find((g: any) => g.name.includes('Toxicology'));
  ...
  if (!s1ProgMatch.ok) throw new Error('S1 Program matching failed: ' + JSON.stringify(s1ProgMatch.data));
  ...
  console.log('     * Addressed Competencies:', s1Prog.skillGaps.map((g: any) => `${g.name}: ${g.recommendation}`));
  ...
  if (!getWeightsRes.ok) throw new Error('Failed to get weights: ' + JSON.stringify(getWeightsRes.data));
  ...
  if (!updateWeightsRes.ok) throw new Error('Failed to update weights: ' + JSON.stringify(updateWeightsRes.data));
  ...
  if (!simRes.ok) throw new Error('Custom calculation failed: ' + JSON.stringify(simRes.data));
  ...
  const finalProgs = await prisma.learningProgram.count();
