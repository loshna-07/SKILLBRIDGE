...
async function runVerification() {
  ...
  const checkCourse = (titleSub: string, providerSub: string) => {
    const c = allCourses.find((x: any) => (x.title || '').includes(titleSub));
  ...
  const oppsData: any = await oppsRes.json();
  const opportunities = Array.isArray(oppsData) ? oppsData : oppsData.opportunities || [];

  const checkOpp = (titleSub: string, type: string) => {
    const o = opportunities.find((x: any) => (x.title || '').includes(titleSub) && x.type === type);
  ...
  const meeraCourses: any = await meeraCoursesRes.json();
  const meeraCourseList = Array.isArray(meeraCourses) ? meeraCourses : meeraCourses.courses || [];
  const meeraDravya = meeraCourseList.find((c: any) => (c.title || c.course?.title || '').includes('Dravyaguna'));
  ...
  const priyaCourses: any = await priyaCoursesRes.json();
  const priyaCourseList = Array.isArray(priyaCourses) ? priyaCourses : priyaCourses.courses || [];
  const priyaEmbed = priyaCourseList.find((c: any) => (c.title || c.course?.title || '').includes('Embedded Systems'));
  ...
  const snehaCourses: any = await snehaCoursesRes.json();
  const snehaCourseList = Array.isArray(snehaCourses) ? snehaCourses : snehaCourses.courses || [];
  const snehaFin = snehaCourseList.find((c: any) => (c.title || c.course?.title || '').includes('Financial Analysis'));
  ...
  const checkInstIntel = async (token: string, instName: string) => {
    ...
    const intelData: any = await intelRes.json();
