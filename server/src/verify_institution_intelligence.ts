...
async function verifyInstitutionIntelligence() {
  ...
  if (initialUserCount !== 0) throw new Error('Database is not empty at start!');
  ...
  if (!emptyIntelRes.ok) throw new Error(`Intelligence endpoint failed: ${JSON.stringify(emptyIntelRes.data)}`);
  ...
  if (!s1Reg.ok) throw new Error(`Student 1 failed: ${JSON.stringify(s1Reg.data)}`);
  ...
  if (!s2Reg.ok) throw new Error(`Student 2 failed: ${JSON.stringify(s2Reg.data)}`);
  ...
  if (!s3Reg.ok) throw new Error(`Student 3 failed: ${JSON.stringify(s3Reg.data)}`);
  ...
  const skillHPLC = await prisma.skill.create({
    data: {
  ...
  const skillStandardization = await prisma.skill.create({
    data: {
  ...
  const skillClinical = await prisma.skill.create({
    data: {
  ...
  const s1Profile = await prisma.studentProfile.findFirst({ where: { user: { email: 'rajesh.kumar@aiia.gov.in' } } });
  const s2Profile = await prisma.studentProfile.findFirst({ where: { user: { email: 'priya.patel@aiia.gov.in' } } });
  const s3Profile = await prisma.studentProfile.findFirst({ where: { user: { email: 'amit.verma@aiia.gov.in' } } });
  ...
  const indProfile = await prisma.industryProfile.findFirst({ where: { user: { email: 'recruitment.pharma@dabur.com' } } });
  ...
  // Opportunity 2: JOB
  const oppJob = await prisma.opportunity.create({
    data: {
      industryId: indProfile!.id,
  ...
  const workshop = await prisma.learningProgram.create({
    data: {
  ...
  if (!intelRes.ok) throw new Error(`Intelligence API failed: ${JSON.stringify(intelRes.data)}`);
  ...
  const stdGap = gaps.find((g: any) => g.skill === 'Standardization of Herbal Extracts');
  ...
  console.log(`     Top Demanded Skills: ${topDemand.map((d: any) => `${d.skill} (${d.totalDemand})`).join(', ')}`);
  ...
  if (
    pipeline.stages[0].count !== 3 || // Applied
  ...
  if (!deptFilterRes.ok) throw new Error('Department filter request failed');
  ...
  if (dravyagunaStudents !== 2) throw new Error('Department filter did not scope students correctly');

  // 10b. Filter by Cohort = 2026
  const cohortFilterRes = await req('GET', '/institution/intelligence?cohort=2026', undefined, instToken);
  if (!cohortFilterRes.ok) throw new Error('Cohort filter request failed');
  ...
  if (cohort2026Students !== 1) throw new Error('Cohort filter did not scope students correctly');
  ...
  if (!oppTypeFilterRes.ok) throw new Error('OpportunityType filter request failed');
  ...
  if (internshipFunnelApps !== 2) throw new Error('OpportunityType filter mismatch');
  ...
  const finalApps = await prisma.application.count();
