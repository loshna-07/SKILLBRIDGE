...
async function main() {
  ...
  try {
    ...
    if (!indReg.ok) throw new Error('Failed to register Industry account: ' + JSON.stringify(indReg.data));
    ...
    if (!acadReg.ok) throw new Error('Failed to register Academician account: ' + JSON.stringify(acadReg.data));
    ...
    if (!studReg.ok) throw new Error('Failed to register Student account: ' + JSON.stringify(studReg.data));
    ...
    if (!instReg.ok) throw new Error('Failed to register Institution account: ' + JSON.stringify(instReg.data));
    ...
    const liveProjectRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!liveProjectRes.ok) throw new Error('Failed to create LIVE_PROJECT: ' + JSON.stringify(liveProjectRes.data));
    const liveProjectId = liveProjectRes.data.collaboration.id;

    const workshopRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!workshopRes.ok) throw new Error('Failed to create WORKSHOP: ' + JSON.stringify(workshopRes.data));
    const workshopId = workshopRes.data.collaboration.id;

    const challengeRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!challengeRes.ok) throw new Error('Failed to create INNOVATION_CHALLENGE: ' + JSON.stringify(challengeRes.data));
    const challengeId = challengeRes.data.collaboration.id;

    const mentorshipRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!mentorshipRes.ok) throw new Error('Failed to create MENTORSHIP: ' + JSON.stringify(mentorshipRes.data));
    const mentorshipId = mentorshipRes.data.collaboration.id;

    // Academician creates: RESEARCH_PROJECT, CONSULTANCY
    const researchRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!researchRes.ok) throw new Error('Failed to create RESEARCH_PROJECT: ' + JSON.stringify(researchRes.data));
    const researchId = researchRes.data.collaboration.id;

    const consultancyRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!consultancyRes.ok) throw new Error('Failed to create CONSULTANCY: ' + JSON.stringify(consultancyRes.data));
    const consultancyId = consultancyRes.data.collaboration.id;

    // Institution creates: INDUSTRY_VISIT, GUEST_LECTURE
    const visitRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!visitRes.ok) throw new Error('Failed to create INDUSTRY_VISIT: ' + JSON.stringify(visitRes.data));
    const visitId = visitRes.data.collaboration.id;

    const lectureRes = await req(
      'POST',
      '/collaboration',
      {
    ...
    if (!lectureRes.ok) throw new Error('Failed to create GUEST_LECTURE: ' + JSON.stringify(lectureRes.data));
    const lectureId = lectureRes.data.collaboration.id;

    // Verify all 8 distinct types in database
    const allCollabsDb = await prisma.collaboration.findMany();
    ...
    const studApplyRes = await req(
      'POST',
      `/collaboration/${liveProjectId}/apply`,
      { proposal: 'I have strong analytical skills in Ayurvedic herbs and wish to contribute.' },
      studToken
    );
    if (!studApplyRes.ok) throw new Error('Student failed to apply to LIVE_PROJECT: ' + JSON.stringify(studApplyRes.data));
    ...
    const studApplyRes2 = await req(
      'POST',
      `/collaboration/${challengeId}/apply`,
      { proposal: 'Interested in AI computer vision for herbal species authentication.' },
      studToken
    );
    if (!studApplyRes2.ok) throw new Error('Student failed to apply to challenge: ' + JSON.stringify(studApplyRes2.data));
    console.log('✔ Student applied to INNOVATION_CHALLENGE');

    // Academician applies to WORKSHOP
    const acadApplyRes = await req(
      'POST',
      `/collaboration/${workshopId}/apply`,
      { proposal: 'I would like to participate in safety monitoring and pharmacovigilance modules.' },
      acadToken
    );
    if (!acadApplyRes.ok) throw new Error('Academician failed to apply to workshop: ' + JSON.stringify(acadApplyRes.data));
    ...
    const acceptRes = await req(
      'PUT',
      `/collaboration/applications/${studAppId}/status`,
      { status: 'ACCEPTED' },
      indToken
    );
    ...
    const rejectRes = await req(
      'PUT',
      `/collaboration/applications/${acadAppId}/status`,
      { status: 'REJECTED' },
      indToken
    );
    ...
    const liveProjectApp = myAppsRes.data.find((a: any) => a.collaborationId === liveProjectId);
    ...
    const feedbackRes = await req(
      'POST',
      `/collaboration/${liveProjectId}/feedback`,
      {
        rating: 5,
    ...
    const remainingApps = await prisma.collaborationApplication.count();
    const remainingFeedbacks = await prisma.collaborationFeedback.count();
