...
const api = {
  ...
  patch: async (url: string, body?: any, config?: { headers?: any }): Promise<{ status: number; data: any }> => {
    const res = await fetch(url, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json', ...(config?.headers || {}) },
      body: body ? JSON.stringify(body) : undefined,
...
async function runLearningHubVerification() {
  ...
  try {
    ...
    const skillCat = await prisma.skillCategory.create({
    ...
    const studentProfile = await prisma.studentProfile.findFirst({ where: { user: { email: 'learner.arjun@ayurveda.edu' } } });
    ...
    const createCourseRes = await api.post(
      `${API_BASE}/courses`,
      {
    ...
    // Add Modules & Lessons
    const mod1Res = await api.post(
      `${API_BASE}/courses/${courseId}/modules`,
      { title: 'Module 1: Principles of Chromatographic Extraction' },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    const mod1Id = mod1Res.data.module.id;

    const lesson1Res = await api.post(
      `${API_BASE}/courses/${courseId}/modules/${mod1Id}/lessons`,
      { title: 'Sample Preparation & Solvent Selection', content: 'Detailed lecture notes on polar vs non-polar solvents.', durationMinutes: 20 },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    const lesson1Id = lesson1Res.data.lesson.id;

    const lesson2Res = await api.post(
      `${API_BASE}/courses/${courseId}/modules/${mod1Id}/lessons`,
      { title: 'TLC Plate Development & Visualization', content: 'Drying and derivatization with vanillin-sulfuric acid reagent.', durationMinutes: 25 },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    const lesson2Id = lesson2Res.data.lesson.id;

    const mod2Res = await api.post(
      `${API_BASE}/courses/${courseId}/modules`,
      { title: 'Module 2: Spectral Analysis & Verification' },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    const mod2Id = mod2Res.data.module.id;

    const lesson3Res = await api.post(
      `${API_BASE}/courses/${courseId}/modules/${mod2Id}/lessons`,
      { title: 'Densitometric Scanning at UV 254nm & 366nm', content: 'Peak identification and Rf value calculation.', durationMinutes: 30 },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    const lesson3Id = lesson3Res.data.lesson.id;

    const lesson4Res = await api.post(
      `${API_BASE}/courses/${courseId}/modules/${mod2Id}/lessons`,
      { title: 'Final Quality Audit & Certification Review', content: 'Documentation for AYUSH compliance.', durationMinutes: 15 },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    ...
    // Publish the course
    await api.patch(
      `${API_BASE}/courses/${courseId}/status`,
      { status: 'PUBLISHED' },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
    ...
    const p1 = await api.post(
      `${API_BASE}/courses/${courseId}/lessons/${lesson1Id}/toggle`,
      { isCompleted: true },
      { headers: { Authorization: `Bearer ${studentToken}` } }
    );
    console.log(`  PASS: Completed Lesson 1 -> Progress: ${p1.data.progressPercentage}% (Expected 25%)`);
    if (p1.data.progressPercentage !== 25) throw new Error('Progress calculation error on step 1.');

    // Complete Lesson 2 (2/4 = 50%)
    const p2 = await api.post(
      `${API_BASE}/courses/${courseId}/lessons/${lesson2Id}/toggle`,
      { isCompleted: true },
      { headers: { Authorization: `Bearer ${studentToken}` } }
    );
    console.log(`  PASS: Completed Lesson 2 -> Progress: ${p2.data.progressPercentage}% (Expected 50%)`);
    if (p2.data.progressPercentage !== 50) throw new Error('Progress calculation error on step 2.');

    // Complete Lesson 3 (3/4 = 75%)
    const p3 = await api.post(
      `${API_BASE}/courses/${courseId}/lessons/${lesson3Id}/toggle`,
      { isCompleted: true },
      { headers: { Authorization: `Bearer ${studentToken}` } }
    );
    console.log(`  PASS: Completed Lesson 3 -> Progress: ${p3.data.progressPercentage}% (Expected 75%)`);
    if (p3.data.progressPercentage !== 75) throw new Error('Progress calculation error on step 3.');

    // Complete Lesson 4 (4/4 = 100%)
    const p4 = await api.post(
      `${API_BASE}/courses/${courseId}/lessons/${lesson4Id}/toggle`,
      { isCompleted: true },
      { headers: { Authorization: `Bearer ${studentToken}` } }
    );
    ...
    const certReqRes = await api.post(
      `${API_BASE}/courses/${courseId}/certificate/request`,
      {},
      { headers: { Authorization: `Bearer ${studentToken}` } }
    );
    ...
    if (cert.verificationStatus !== 'PENDING') throw new Error('Certificate was not in PENDING status.');
    ...
    const approveRes = await api.post(
      `${API_BASE}/courses/${courseId}/certificates/${cert.id}/approve`,
      { status: 'VERIFIED', remarks: 'All laboratory assays verified with excellence.' },
      { headers: { Authorization: `Bearer ${acadToken}` } }
    );
