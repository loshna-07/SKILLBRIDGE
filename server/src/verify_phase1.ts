...
async function verifyPhase1() {
  ...
  const dbResult: any = await prisma.$queryRawUnsafe('SELECT version();');
  ...
  const acadCount = await prisma.academicianProfile.count();
  const indCount = await prisma.industryProfile.count();
  const instCount = await prisma.institutionProfile.count();
  ...
  const studentDb = await prisma.user.findUnique({
    where: { email: studentPayload.email.toLowerCase() },
    include: { studentProfile: true },
  });
  if (!studentDb) throw new Error('Student record not found in PostgreSQL!');
  ...
  const acadDb = await prisma.user.findUnique({
    where: { email: acadPayload.email.toLowerCase() },
    include: { academicianProfile: true },
  });
  if (!acadDb) throw new Error('Academician record not found in PostgreSQL!');
  ...
  if (!indDb) throw new Error('Industry record not found in PostgreSQL!');
  ...
  const instDb = await prisma.user.findUnique({
    where: { email: instPayload.email.toLowerCase() },
    include: { institutionProfile: true },
  });
  if (!instDb) throw new Error('Institution record not found in PostgreSQL!');
  ...
  for (const acc of [
    ...
    if (loginRes.data.user.role !== acc.role) throw new Error(`Role mismatch for ${acc.role}`);
  ...
  if (!meStudent.ok) throw new Error('Failed to resolve /api/auth/me for student');
  ...
  if (!meInst.ok) throw new Error('Failed to resolve /api/auth/me for institution');
