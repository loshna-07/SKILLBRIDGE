...
async function verifyRecruitmentSystem() {
  ...
  const skillFormulation = await prisma.skill.create({
    data: {
  ...
  const skillPharma = await prisma.skill.create({
    data: {
      categoryId: category.id,
  ...
  const foundInUnpublished = browseUnpublished.data?.some((o: any) => o.id === oppId);
  if (foundInUnpublished) throw new Error('Unpublished opportunity appeared in public list!');
  ...
  const matchedOpp = studentBrowseRes.data.find((o: any) => o.id === oppId);
  if (!matchedOpp) throw new Error('Created opportunity not found in student browse list!');
  ...
  console.log('  -> Matched Skills:', matchedOpp.matchResult?.matchedSkills?.map((s: any) => s.name));
  ...
  const applicant = applicantsRes.data.find((a: any) => a.id === applicationId);
  if (!applicant) throw new Error('Applicant not found in recruiter view!');
  ...
  const trackedApp = studentAppsRes.data.find((a: any) => a.id === applicationId);
  if (!trackedApp) throw new Error('Tracked application not found in student pipeline!');
  ...
  trackedApp.history.forEach((h: any, idx: number) => {
    console.log(`    ${idx + 1}. [${new Date(h.createdAt).toLocaleTimeString()}] Status: ${h.status.padEnd(12)} - Note: ${h.notes}`);
  });

  const statusesInOrder = trackedApp.history.map((h: any) => h.status);
  if (
    !statusesInOrder.includes('APPLIED') ||
  ...
  if (!dummyOpp.ok || !dummyOpp.data?.id) throw new Error('Failed to create dummy opportunity');

  const deleteRes = await req('DELETE', `/opportunities/${dummyOpp.data.id}`, undefined, industryToken);
  if (!deleteRes.ok) throw new Error(`Failed to delete opportunity: ${JSON.stringify(deleteRes.data)}`);
