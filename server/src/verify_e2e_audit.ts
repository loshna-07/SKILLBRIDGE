...
async function req(method: string, path: string, body?: any, token?: string): Promise<ApiResponse> {
...
async function runAudit() {
  ...
  const profSkill = skillsRes.data.skillProfiles.find((s: any) => s.skillId === skill1.id);
  ...
  const matchingRec = recRes.data?.recommendations?.find((r: any) => r.id === programId);
  ...
  const matchedOpp = studentOpps.data?.find((o: any) => o.id === opportunityId);
  ...
  const studentApp = studentProfileCheck.data?.applications?.find((a: any) => a.id === applicationId);
  ...
  const oppInList = acadOpps.data?.find((o: any) => o.id === collabId);
