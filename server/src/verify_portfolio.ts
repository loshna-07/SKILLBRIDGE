...
async function req(method: string, endpoint: string, body?: any, token?: string): Promise<{ status: number; ok: boolean; data: any }> {
...
async function verifyPortfolio() {
  ...
  const eduCount = await prisma.studentEducation.count();
  const projCount = await prisma.studentProject.count();
  console.log(`Database verified clean. Users: ${userCount}, Educations: ${eduCount}, Projects: ${projCount}`);
  if (userCount !== 0) throw new Error('Database is not clean at start!');
  ...
  if (!submitRes.data.passed) throw new Error('Expected assessment to pass!');

  // Strict Rule Check: Ensure StudentSkillProfile created by assessment is NOT automatically verified!
  const assessedSkillProfile = await prisma.studentSkillProfile.findFirst({
  ...
  // 1. Education
  const eduRes = await req(
    'POST',
    '/student/portfolio/education',
    {
  ...
  if (eduRes.data.verificationStatus !== 'PENDING') throw new Error('Education not PENDING!');

  // 2. Skill self-addition
  const skillRes = await req(
    'POST',
  ...
  if (certRes.data.verificationStatus !== 'PENDING') throw new Error('Certification not PENDING!');
  ...
  if (projRes.data.verificationStatus !== 'PENDING') throw new Error('Project not PENDING!');

  // 5. Internship
  const internRes = await req(
    'POST',
    '/student/portfolio/internships',
    {
  ...
  if (internRes.data.verificationStatus !== 'PENDING') throw new Error('Internship not PENDING!');

  // 6. Achievement
  const achievRes = await req(
    'POST',
  ...
  if (achievRes.data.verificationStatus !== 'PENDING') throw new Error('Achievement not PENDING!');

  // 7. Training
  const trainRes = await req(
    'POST',
    '/student/portfolio/trainings',
    {
  ...
  if (trainRes.data.verificationStatus !== 'PENDING') throw new Error('Training not PENDING!');
  ...
  if (resumeRes.data.profile.resumeVerificationStatus !== 'PENDING') throw new Error('Resume not PENDING!');
  ...
  console.log(`- Skills: ${port.skillProfiles.length} (statuses: ${port.skillProfiles.map((s: any) => s.verificationStatus).join(', ')})`);
  ...
  if (port.educations.length !== 1) throw new Error('Expected 1 education');
  if (port.skillProfiles.length !== 2) throw new Error('Expected 2 skill profiles');
  if (port.certificates.length !== 1) throw new Error('Expected 1 certificate');
  if (port.projects.length !== 1) throw new Error('Expected 1 project');
  if (port.internships.length !== 1) throw new Error('Expected 1 internship');
  if (port.achievements.length !== 1) throw new Error('Expected 1 achievement');
  if (port.trainings.length !== 1) throw new Error('Expected 1 training');
  if (port.assessmentAttempts.length !== 1) throw new Error('Expected 1 assessment attempt');
  if (!port.resumeUrl) throw new Error('Expected resume URL');

  // Verify zero automatic verifications
  const allStatuses = [
    port.educations[0].verificationStatus,
    ...port.skillProfiles.map((s: any) => s.verificationStatus),
  ...
  // Verify Skill (skill2)
  const skillProfileToVerify = port.skillProfiles.find((s: any) => s.skillId === skill2.id)!;
  await req(
    'POST',
    '/institution/portfolio/verify',
    {
  ...
  if (upPort.educations[0].verificationStatus !== 'VERIFIED') throw new Error('Expected Education to be VERIFIED');

  // Project should be VERIFIED
  console.log(`Project verificationStatus: ${upPort.projects[0].verificationStatus}`);
  if (upPort.projects[0].verificationStatus !== 'VERIFIED') throw new Error('Expected Project to be VERIFIED');

  // Skill2 should be VERIFIED and verified: true
  const verifiedSkill = upPort.skillProfiles.find((s: any) => s.skillId === skill2.id)!;
  ...
  if (upPort.trainings[0].verificationStatus !== 'VERIFIED') throw new Error('Expected Training to be VERIFIED');

  // Resume should be VERIFIED
  console.log(`Resume verificationStatus: ${upPort.resumeVerificationStatus}`);
  if (upPort.resumeVerificationStatus !== 'VERIFIED') throw new Error('Expected Resume to be VERIFIED');
  ...
  if (upPort.certificates[0].verificationStatus !== 'REJECTED') throw new Error('Expected Certification to be REJECTED');
  ...
  if (upPort.achievements[0].verificationStatus !== 'PENDING') throw new Error('Expected Achievement to remain PENDING');
  ...
  const skill1Profile = upPort.skillProfiles.find((s: any) => s.skillId === skill1.id)!;
  ...
  if (publicRes.data.certificates.length !== 0) throw new Error('Deleted certificate still in portfolio!');
  if (publicRes.data.skillProfiles.length !== 1) throw new Error('Deleted skill still in portfolio!');
  ...
  const finalEdu = await prisma.studentEducation.count();
