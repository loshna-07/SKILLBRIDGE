...
async function verifyLearningModule() {
  ...
  const progCount = await prisma.learningProgram.count();
  ...
  const skillC = await prisma.skill.create({
    data: {
      categoryId: category.id,
  ...
  if (!pTraining.ok) throw new Error(`Failed to create Training: ${JSON.stringify(pTraining.data)}`);
  console.log(`  -> 1. TRAINING created: "${pTraining.data.title}" (teaches ${pTraining.data.skills?.map((s: any) => s.skill.name).join(', ')})`);
  ...
  if (!pCert.ok) throw new Error(`Failed to create Certification: ${JSON.stringify(pCert.data)}`);
  console.log(`  -> 2. CERTIFICATION created: "${pCert.data.title}" (teaches ${pCert.data.skills?.map((s: any) => s.skill.name).join(', ')})`);
  ...
  if (!pWorkshop.ok) throw new Error(`Failed to create Workshop: ${JSON.stringify(pWorkshop.data)}`);
  console.log(`  -> 3. WORKSHOP created: "${pWorkshop.data.title}" (teaches ${pWorkshop.data.skills?.map((s: any) => s.skill.name).join(', ')})`);
  ...
  if (!pBootcamp.ok) throw new Error(`Failed to create Bootcamp: ${JSON.stringify(pBootcamp.data)}`);
  console.log(`  -> 4. BOOTCAMP created: "${pBootcamp.data.title}" (teaches ${pBootcamp.data.skills?.map((s: any) => s.skill.name).join(', ')})`);
  ...
  if (!pMentorship.ok) throw new Error(`Failed to create Mentorship: ${JSON.stringify(pMentorship.data)}`);
  console.log(`  -> 5. MENTORSHIP created: "${pMentorship.data.title}" (teaches ${pMentorship.data.skills?.map((s: any) => s.skill.name).join(', ')})`);
  ...
  if (!industryOpp.ok) throw new Error('Failed to create opportunity');
  ...
  skillGaps.forEach((g: any, i: number) => {
    console.log(`     ${i + 1}. [${g.gapType}] ${g.skillName} (Current: ${g.currentScore}%, Target: ${g.targetScore}%)`);
    console.log(`        Targeted DB Programs Attached: ${g.recommendedPrograms?.map((p: any) => p.title).join(' | ')}`);
  });

  const gapSkillNames = skillGaps.map((g: any) => g.skillName);
  ...
  recommendedPrograms.forEach((p: any, idx: number) => {
  ...
  const recommendedIds = recommendedPrograms.map((p: any) => p.id);
  ...
  const enrolledBootcamp = industryCheck.data.find((p: any) => p.id === pBootcamp.data.id);
  ...
  if (!tempProg.ok) throw new Error('Failed to create temp program');

  const deleteRes = await req('DELETE', `/learning/${tempProg.data.id}`, undefined, industryToken);
  if (!deleteRes.ok) throw new Error('Failed to delete program');
  ...
  const finalEnrollments = await prisma.learningEnrollment.count();
