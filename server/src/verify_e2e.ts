...
const api = {
  get: (path: string, token?: string): Promise<any> => req('GET', path, undefined, token),
  post: (path: string, body: any, token?: string): Promise<any> => req('POST', path, body, token),
  put: (path: string, body: any, token?: string): Promise<any> => req('PUT', path, body, token),
  delete: (path: string, token?: string): Promise<any> => req('DELETE', path, undefined, token),
};
...
async function runVerification() {
  ...
  const catRes = await api.post(
    ...
    { name: `Full-Stack Development ${Date.now()}`, description: 'Web application technologies' },
    instToken
  );
  const categoryId = catRes.id;

  const skill1Res = await api.post(
    '/assessments/skills',
    { categoryId, name: `React.js-${Date.now()}`, description: 'Frontend library' },
    instToken
  );
  const skillReactId = skill1Res.id;

  const skill2Res = await api.post(
    '/assessments/skills',
    { categoryId, name: `Node.js-${Date.now()}`, description: 'Backend runtime' },
    instToken
  );
  const skillNodeId = skill2Res.id;

  const skill3Res = await api.post(
    '/assessments/skills',
    { categoryId, name: `Docker-${Date.now()}`, description: 'Containerization' },
    instToken
  );
  ...
  const assessRes = await api.post(
    '/assessments',
    {
  ...
  const responses = questions.map((q: any) => {
    ...
    const correctOpt = q.options.find((o: any) => o.optionText.includes('useEffect') || o.optionText.includes('CommonJS')) || q.options[0];
    return {
      questionId: q.id,
  ...
  const targetOpp = oppsListRes.find((o: any) => o.id === opportunityId);
  ...
  const applyRes = await api.post(
    ...
    { coverLetter: 'I have experience building React and Node.js applications.' },
    studentToken
  );
  ...
  await api.post(
    `/opportunities/applications/${applicationId}/interview`,
    {
  ...
  const selectRes = await api.put(
    `/opportunities/applications/${applicationId}/status`,
    { status: 'SELECTED', notes: 'Candidate selected for 6-month internship.' },
    indToken
  );
