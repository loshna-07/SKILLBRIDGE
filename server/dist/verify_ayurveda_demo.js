...
async function verifyAyurvedaEcosystem() {
    ...
    const pkOpp = (ananyaOpps.data || []).find((o) => o.title.includes('Panchakarma Therapy Intern'));
    ...
    const ananyaPkApp = (ananyaApps.data || []).find((a) => a.opportunity?.title?.includes('Panchakarma Therapy Intern'));
    ...
    const rohanPkOpp = (rohanOpps.data || []).find((o) => o.title?.includes('Panchakarma Therapy Intern'));
    ...
    const dravyagunaCourse = (meeraEnrollments.data || []).find((e) => e.course?.title?.includes('Dravyaguna'));
    ...
    const fdp = (collabsRes.data || []).find((c) => c.type === 'FDP');
