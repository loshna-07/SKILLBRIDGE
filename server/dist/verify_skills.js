...
async function verifySkillsFlow() {
    ...
    if (!catRes.ok)
        throw new Error('Failed to create skill category: ' + JSON.stringify(catRes.data));
    ...
    if (!skill1Res.ok)
        throw new Error('Failed to create skill 1: ' + JSON.stringify(skill1Res.data));
    ...
    if (!skill2Res.ok)
        throw new Error('Failed to create skill 2: ' + JSON.stringify(skill2Res.data));
    ...
    if (!listSkills.ok || listSkills.data.length < 2)
        throw new Error('Skills endpoint failed');
    ...
    if (!assessRes.ok)
        throw new Error('Failed to create assessment: ' + JSON.stringify(assessRes.data));
    ...
    if (!updateProfileRes.ok)
        throw new Error('Student profile update failed: ' + JSON.stringify(updateProfileRes.data));
    ...
    if (!getTestRes.ok)
        throw new Error('Failed to fetch assessment: ' + JSON.stringify(getTestRes.data));
    ...
    const responsesToSubmit = [
        { questionId: dbQuestions[0].id, selectedOptionId: dbQuestions[0].options.find((o) => o.isCorrect).id },
        { questionId: dbQuestions[1].id, selectedOptionId: dbQuestions[1].options.find((o) => o.isCorrect).id },
        { questionId: dbQuestions[2].id, selectedOptionId: dbQuestions[2].options.find((o) => !o.isCorrect).id },
        { questionId: dbQuestions[3].id, selectedOptionId: dbQuestions[3].options.find((o) => !o.isCorrect).id },
    ];
    const submitRes = await req('POST', `/assessments/${assessmentId}/submit`, { responses: responsesToSubmit }, studentToken);
    if (!submitRes.ok)
        throw new Error('Failed to submit assessment: ' + JSON.stringify(submitRes.data));
    ...
    const attempt = await db_1.default.assessmentAttempt.findUnique({
        where: { id: result.attemptId },
        include: { responses: true },
    });
    if (!attempt)
        throw new Error('AssessmentAttempt not found in DB');
    ...
    const skillProfiles = await db_1.default.studentSkillProfile.findMany({
    ...
    if (!gapRes.ok)
        throw new Error('Failed to fetch skill gap: ' + JSON.stringify(gapRes.data));
    ...
    const pulseGap = gapData.skillGaps.find((g) => g.skillName === 'Clinical Pulse Diagnosis (Nadi Pariksha)');
    ...
    console.log('  -> Strengths identified:', gapData.strengths.map((s) => s.name));
    console.log('  -> Weaknesses identified:', gapData.weaknesses.map((w) => w.name));
    console.log('  -> Skill Gaps identified:', gapData.skillGaps.map((g) => g.skillName + ' (' + g.gapType + ', ' + g.severity + ' PRIORITY)'));
    // 14. Clean Teardown
    ...
    const finalAssessments = await db_1.default.assessment.count();
