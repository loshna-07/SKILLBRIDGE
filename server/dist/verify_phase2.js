...
async function verifyPhase2() {
    ...
    const initialUsers = await db_1.default.user.count();
    ...
    if (!acadDash.ok)
        throw new Error(`Academician dashboard failed: ${JSON.stringify(acadDash.data)}`);
    ...
    if (!indOpps.ok)
        throw new Error(`Industry opportunities failed: ${JSON.stringify(indOpps.data)}`);
    if (!indApps.ok)
        throw new Error(`Industry applicants failed: ${JSON.stringify(indApps.data)}`);
    ...
    if (!instDash.ok)
        throw new Error(`Institution dashboard failed: ${JSON.stringify(instDash.data)}`);
    ...
    for (const item of [
        ...
        if (!meRes.ok)
            throw new Error(`Failed /auth/me for ${item.role}`);
    ...
    await db_1.default.user.deleteMany({});
    const finalCount = await db_1.default.user.count();
