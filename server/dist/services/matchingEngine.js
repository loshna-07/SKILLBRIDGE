...
exports.calculateLearningProgramMatch = exports.calculateOpportunityCompatibility = exports.checkOpportunityEligibility = exports.calculateOpportunityMatch = exports.isDegreeMatch = exports.isDepartmentMatch = exports.getPlatformWeights = exports.PROFICIENCY_RANK = void 0;
const db_1 = __importDefault(require("../config/db"));
exports.PROFICIENCY_RANK = {
    BEGINNER: 1,
    DEVELOPING: 2,
    INTERMEDIATE: 3,
    ADVANCED: 4,
...
const normalizeAcademicString = (str) => {
    return (str || '')
        .toLowerCase()
        .replace(/[().,;:/|_\-]/g, ' ')
        .replace(/\s+/g, ' ')
...
const DEPARTMENT_SYNONYM_GROUPS = [
    // Computer Science & IT
...
const isDepartmentMatch = (studentDept, oppDept) => {
...
exports.isDepartmentMatch = isDepartmentMatch;
const isDegreeMatch = (studentDegree, oppDegree) => {
...
exports.isDegreeMatch = isDegreeMatch;
/**
 * Calculate compatibility between a Student and an Opportunity (Internship or Job)
 */
const calculateOpportunityMatch = async (studentId, opportunityId, customWeights) => {
    ...
    const defaultWeights = customWeights || (await (0, exports.getPlatformWeights)());
    ...
    for (const oppSkill of opportunity.skills) {
        ...
        const requiredRank = exports.PROFICIENCY_RANK[requiredProficiency] || 3;
        if (oppSkill.isRequired) {
            totalRequiredCount++;
            if (studentSkill) {
                requiredMatchedCount++;
                const studentRank = exports.PROFICIENCY_RANK[studentSkill.proficiency] || 1;
                matchedSkills.push({
                ...
                if (studentRank < requiredRank) {
                    proficiencyEligible = false;
                    ineligibleReasons.push(`Mandatory skill required: ${skillName} (Required proficiency: ${requiredProficiency}, your proficiency: ${studentSkill.proficiency}).`);
                    skillGaps.push({
                        skillId: oppSkill.skillId,
                        name: skillName,
                ...
                ineligibleReasons.push(`Mandatory skill required: ${skillName} (Required proficiency: ${requiredProficiency}, your proficiency: Not added).`);
                skillGaps.push({
                    skillId: oppSkill.skillId,
                    name: skillName,
    ...
    const departmentEligible = (0, exports.isDepartmentMatch)(student.department, opportunity.department);
    ...
    const degreeEligible = (0, exports.isDegreeMatch)(student.degree, opportunity.degree);
    if (!degreeEligible) {
...
const checkOpportunityEligibility = async (studentId, opportunityId) => {
    const match = await (0, exports.calculateOpportunityMatch)(studentId, opportunityId);
    return match.eligibility;
};
exports.checkOpportunityEligibility = checkOpportunityEligibility;
const calculateOpportunityCompatibility = async (studentId, opportunityId) => {
    const match = await (0, exports.calculateOpportunityMatch)(studentId, opportunityId);
    return match.matchScore;
};
exports.calculateOpportunityCompatibility = calculateOpportunityCompatibility;
/**
 * Calculate compatibility between a Student and a Learning Program
 */
const calculateLearningProgramMatch = async (studentId, programId, customWeights) => {
    const student = await db_1.default.studentProfile.findUnique({
        where: { id: studentId },
        include: {
            skillProfiles: {
    ...
    const weights = customWeights || (await (0, exports.getPlatformWeights)());
    if (!student || !program) {
        return {
            matchScore: 0,
            matchPercentage: 0,
    ...
    const avgAssessmentScore = student.skillProfiles.length > 0
        ? Math.round(student.skillProfiles.reduce((acc, curr) => acc + (curr.scorePercentage || 0), 0) /
            student.skillProfiles.length)
        : 0;
    const studentCgpaScore = student.cgpa ? Math.min(100, Math.round((Number(student.cgpa) / 10) * 100)) : 0;
    const explanation = gapsAddressedCount > 0
...
exports.calculateLearningProgramMatch = calculateLearningProgramMatch;
