...
exports.getInstitutionAnalytics = exports.getInstitutionIntelligence = exports.getPublicPlatformStats = void 0;
const db_1 = __importDefault(require("../config/db"));
const getPublicPlatformStats = async () => {
    const [totalStudents, totalAcademicians, totalCompanies, totalInstitutions, totalCourses, totalInternships, totalJobs, totalCollaborations, totalOpportunities,] = await Promise.all([
        ...
        db_1.default.course.count({ where: { status: 'PUBLISHED' } }),
        db_1.default.opportunity.count({ where: { isPublished: true, type: 'INTERNSHIP' } }),
        db_1.default.opportunity.count({ where: { isPublished: true, type: 'JOB' } }),
...
const getInstitutionIntelligence = async (filters = {}, institutionName) => {
    ...
    const oppWhere = { isPublished: true };
    if (opportunityType && opportunityType !== 'ALL') {
        oppWhere.type = opportunityType.toUpperCase();
    }
    // 1. Overall counts to determine hasData
    const [totalStudentsAll, totalOpportunitiesAll, totalCollaborationsAll, totalApplicationsAll] = await Promise.all([
        db_1.default.studentProfile.count({ where: institutionName ? { institutionName: { contains: institutionName, mode: 'insensitive' } } : {} }),
        db_1.default.opportunity.count({ where: { isPublished: true } }),
        db_1.default.collaboration.count(),
        db_1.default.application.count(),
    ]);
    const hasData = totalStudentsAll > 0 ||
    ...
    for (const sp of studentSkillProfiles) {
        const prof = (sp.proficiencyLevel || 'BEGINNER').toUpperCase();
        ...
        if (sp.verified)
            skillCountMap[sName].verifiedCount++;
    ...
    const workModeDemandMap = { ON_SITE: 0, REMOTE: 0, HYBRID: 0 };
    for (const os of opportunitySkills) {
        ...
        if (os.opportunity.type === 'INTERNSHIP')
            industryDemandMap[sName].internship++;
        if (os.opportunity.type === 'JOB')
            industryDemandMap[sName].job++;
    ...
    const allUniqueSkills = Array.from(new Set([...Object.keys(skillCountMap), ...Object.keys(industryDemandMap)]));
    ...
    const internshipApplications = await db_1.default.application.findMany({
        where: {
    ...
    const internshipDeptMap = {};
    const hiringCompaniesMap = {};
    ...
    const internshipSelectionRate = totalInternshipApps > 0 ? Math.round((selectedInternshipsCount / totalInternshipApps) * 100) : 0;
    ...
    const pipelineAppWhere = { student: studentWhere };
    if (opportunityType && opportunityType !== 'ALL') {
        pipelineAppWhere.opportunity = { type: opportunityType.toUpperCase() };
    }
    const allPipelineApps = await db_1.default.application.findMany({
        where: pipelineAppWhere,
        select: { status: true },
    });
    const totalPipelineApps = allPipelineApps.length;
    const shortlistedApps = allPipelineApps.filter((a) => ['SHORTLISTED', 'INTERVIEW', 'SELECTED'].includes(a.status)).length;
    const interviewApps = allPipelineApps.filter((a) => ['INTERVIEW', 'SELECTED'].includes(a.status)).length;
    ...
    const learningEnrollments = await db_1.default.learningEnrollment.findMany({
        where: { student: studentWhere },
        include: {
    ...
    for (const enr of learningEnrollments) {
        ...
        const eStatus = enr.status?.toUpperCase();
    ...
    const learningParticipation = {
        ...
        completionRate: totalEnrollments > 0 ? Math.round((enrollmentStatusCounts.COMPLETED / totalEnrollments) * 100) : 0,
    };
    // 7. Dimension 8: Industry Collaboration
    const [collaborations, collaborationApplications] = await Promise.all([
        db_1.default.collaboration.findMany({
    ...
    for (const s of allStudents) {
        ...
        if (s.assessmentAttempts.length > 0) {
            ...
            const avgStudentScore = s.assessmentAttempts.reduce((sum, a) => sum + a.percentage, 0) / s.assessmentAttempts.length;
    ...
    const departmentReadiness = Object.entries(departmentMap)
        ...
        const avgSkillsPerStudent = d.studentCount > 0 ? Number((d.totalSkills / d.studentCount).toFixed(1)) : 0;
        // Composite Readiness Index: weighted 40% avgScore + 30% assessmentRate + 30% selectionRate
        const readinessIndex = Math.min(100, Math.max(0, Math.round(avgScore * 0.4 + assessmentRate * 0.3 + Math.min(100, selectionRate * 2) * 0.3)));
        return {
            department: dept,
    ...
    const overallAssessmentRate = totalStudents > 0 ? Math.round((overallAssessedStudents / totalStudents) * 100) : 0;
    const allCompletedAttempts = allStudents.flatMap((s) => s.assessmentAttempts);
    const overallAvgScore = allCompletedAttempts.length > 0
...
exports.getInstitutionIntelligence = getInstitutionIntelligence;
// Backward compatible helper for existing dashboard endpoint
const getInstitutionAnalytics = async (institutionName) => {
    const intel = await (0, exports.getInstitutionIntelligence)({}, institutionName);
    return {
