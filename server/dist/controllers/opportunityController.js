...
exports.scheduleInterview = exports.updateApplicationStatus = exports.getOpportunityApplicants = exports.applyToOpportunity = exports.submitOpportunityAssessment = exports.getOpportunityAssessment = exports.getMyOpportunities = exports.deleteOpportunity = exports.updateOpportunity = exports.createOpportunity = exports.getOpportunityById = exports.getOpportunities = exports.ASSESSMENT_PASSING_SCORE = void 0;
exports.ASSESSMENT_PASSING_SCORE = 75.0;
...
const mapPercentageToProficiency = (percentage) => {
    if (percentage >= 90)
        return 'EXPERT';
    if (percentage >= 75)
        return 'ADVANCED';
    ...
    try {
        ...
        const enhanced = await Promise.all(opportunities.map(async (opp) => {
            ...
            if (studentId) {
                ...
                if (opp.assessment) {
                    latestAssessmentAttempt = await db_1.default.assessmentAttempt.findFirst({
                        where: {
                            studentId,
...
const createOpportunity = async (req, res) => {
    try {
        ...
        const { title, type, description, degree, department, minCgpa, experience, location, workMode, stipendOrSalary, applicationDeadline, numberOfOpenings, duration, startDate, responsibilities, selectionProcess, eligibleDepartments, eligibleDegrees, skillIds, requiredSkills, preferredSkills, assessmentRequired, assessmentData, assessment, } = req.body;
        ...
        if (isAssessmentRequired && rawAssessment) {
            ...
            for (let i = 0; i < rawAssessment.questions.length; i++) {
                ...
                const hasCorrect = q.options.some((opt) => Boolean(opt.isCorrect));
                if (!hasCorrect) {
            ...
            if (isAssessmentRequired && rawAssessment) {
                ...
                const createdAssessment = await tx.assessment.create({
                    data: {
                        ...
                        durationMinutes,
                        passingScore: exports.ASSESSMENT_PASSING_SCORE,
                        isLocked: false,
                    },
                });
        ...
        const fullOpp = await db_1.default.opportunity.findUnique({
            where: { id: opp.id },
            include: {
...
const updateOpportunity = async (req, res) => {
    try {
        ...
        const { title, type, description, degree, department, minCgpa, experience, location, workMode, stipendOrSalary, applicationDeadline, numberOfOpenings, duration, startDate, responsibilities, selectionProcess, isPublished, skillIds, assessmentRequired, assessmentData, assessment, } = req.body;
        const rawAssessment = assessmentData || assessment;
...
const getOpportunityAssessment = async (req, res) => {
    try {
        ...
        if (isStudent) {
            ...
            if (student) {
                studentAttempt = await db_1.default.assessmentAttempt.findFirst({
...
exports.getOpportunityAssessment = getOpportunityAssessment;
// Student Submits Opportunity-Specific Assessment Attempt
const submitOpportunityAssessment = async (req, res) => {
    try {
        const opportunityId = req.params.id;
        ...
        const passed = percentage >= (assessment.passingScore || exports.ASSESSMENT_PASSING_SCORE);
        ...
        for (const [sId, stats] of skillScoreMap.entries()) {
            ...
            if (isStr)
                ...
                developingSkills.push(stats.name);
            else
                skillGaps.push(stats.name);
        }
        ...
        opp.skills.forEach((os) => {
            if (!strengths.includes(os.skill.name) && !skillGaps.includes(os.skill.name)) {
                ...
                if (!studentSkill || studentSkill.proficiencyLevel === 'BEGINNER') {
                    if (!skillGaps.includes(os.skill.name))
                        skillGaps.push(os.skill.name);
                }
            }
        });
        // Calculate Comprehensive Compatibility Score
        const careerInterestsList = (0, personalizationService_1.parseKeywordList)(student.careerInterests);
        const isCareerMatch = careerInterestsList.some((ci) => (0, personalizationService_1.isKeywordMatch)(ci, opp.title + ' ' + opp.description));
        let compatibilityScore = Math.round(percentage * 0.6 +
            (isCareerMatch ? 25 : 10) +
        ...
        // Notify Recruiter
        const indProfile = await db_1.default.industryProfile.findUnique({
        ...
        res.status(201).json({
            ...
            passingScore: assessment.passingScore || exports.ASSESSMENT_PASSING_SCORE,
            compatibilityScore,
            matchScore: compatibilityScore,
            strengths,
            demonstratedStrengths: strengths,
...
exports.submitOpportunityAssessment = submitOpportunityAssessment;
...
const applyToOpportunity = async (req, res) => {
    try {
        ...
        if (opp.assessmentRequired && opp.assessment) {
            const attempt = await db_1.default.assessmentAttempt.findFirst({
                where: { studentId: student.id, assessmentId: opp.assessment.id },
                orderBy: { startedAt: 'desc' },
            });
...
const getOpportunityApplicants = async (req, res) => {
    try {
        ...
        if (minScore) {
            whereClause.assessmentScore = { gte: parseFloat(minScore) };
        }
        if (minCompatibility) {
            whereClause.matchScore = { gte: parseFloat(minCompatibility) };
        }
        let applications = await db_1.default.application.findMany({
            where: whereClause,
            include: {
                student: {
                    include: {
        ...
        if (skillId && typeof skillId === 'string' && skillId !== 'ALL') {
            applications = applications.filter((app) => app.student?.skillProfiles?.some((sp) => sp.skillId === skillId));
        }
