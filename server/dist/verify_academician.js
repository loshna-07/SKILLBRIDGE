...
async function verifyAcademicianPortal() {
    ...
    const initialCollabCount = await db_1.default.collaboration.count();
    const initialCollabAppCount = await db_1.default.collaborationApplication.count();
    ...
    const applyInternship = await req('POST', `/academician/opportunities/${facultyInternship.id}/apply`, {
        proposal: 'I have 12 years of research experience in TLC/HPTLC fingerprinting of Ayurvedic herbs. I aim to utilize this sabbatical to standardize volatile markers in Dashamoola raw materials.',
    }, acadToken);
    ...
    // Apply to FDP
    const applyFdp = await req('POST', `/academician/opportunities/${fdpOpportunity.id}/apply`, {
    ...
    const duplicateApply = await req('POST', `/academician/opportunities/${facultyInternship.id}/apply`, { proposal: 'Duplicate attempt' }, acadToken);
    ...
    if (!dashboard.ok)
        throw new Error(`Dashboard failed: ${JSON.stringify(dashboard.data)}`);
    ...
    const applicantsRes = await req('GET', `/academician/opportunities/${facultyInternship.id}/applicants`, undefined, indToken);
    ...
    const updateStatusRes = await req('PUT', `/academician/applications/${applicantRecord.id}/status`, { status: 'ACCEPTED' }, indToken);
    ...
    const acceptedPart = updatedParticipations.data.find((p) => p.collaborationId === facultyInternship.id);
    ...
    const withdrawRes = await req('POST', `/academician/participations/${fdpOpportunity.id}/withdraw`, undefined, acadToken);
    if (!withdrawRes.ok) {
    ...
    const withdrawnPart = postWithdrawParticipations.data.find((p) => p.collaborationId === fdpOpportunity.id);
    ...
    // Edit Opportunity
    const editRes = await req('PUT', `/academician/opportunities/${consultancyOpp.id}`, {
    ...
    // Delete Opportunity
    const deleteRes = await req('DELETE', `/academician/opportunities/${consultancyOpp.id}`, undefined, acadToken);
    if (!deleteRes.ok) {
    ...
    const finalCollabs = await db_1.default.collaboration.count();
    const finalApps = await db_1.default.collaborationApplication.count();
